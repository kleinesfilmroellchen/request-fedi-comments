...
/// * While yaserde is broken, make title, current_status and publication_status of RfcEntry optional. (No idea why, the XML doesn't *actually* seem to do that but yaserde thinks so.)
///
...
pub async fn fetch_random_rfc() -> Result<RfcEntry, Error> {
	let rfc_list = fetch_rfc_list().await?;
	Ok(rfc_list.into_iter().choose(&mut thread_rng()).unwrap())
}
